...
| 觸發源 | 範例場景 | AOS 介面 | OSS 實作 |
|--------|----------|----------|----------|
| 排程 / cron | 每日掃描報表 | `Trigger.cron` | `croner` |
| Kafka 事件 | 訂單建立、庫存變動 | `Trigger.kafka` | `kafkajs` |
| 人類訊息（Slack / IM） | 「幫我看一下這個」 | `Trigger.slack` | `@slack/bolt` |
| 報表 / 異常（BI / 監控） | 數字異常、開 ticket | 自寫輪詢觸發 | — |
| 外部 API / Webhook | GitHub PR、Jira issue、ERP event | 自寫 webhook endpoint | — |
| 同儕 AOS 訊息 | AOS-A 通知 AOS-B 接手 | `Trigger.kafka` 變體 | `kafkajs` |
